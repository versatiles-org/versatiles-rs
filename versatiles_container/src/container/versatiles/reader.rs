...
mod tests {
	use super::*;
	use crate::{MOCK_BYTES_PBF, MockTilesReader, TilesWriterTrait, VersaTilesWriter, WriterConfig, make_test_file};
	...
	async fn read_your_own_dog_food() -> Result<()> {
		...
		VersaTilesWriter::write_to_writer(&mut reader1, &mut data_writer1, WriterConfig::default().arc()).await?;
		...
		VersaTilesWriter::write_to_writer(&mut reader2, &mut data_writer2, WriterConfig::default().arc()).await?;
