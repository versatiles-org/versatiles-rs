...
mod tests {
	...
	async fn reader() -> Result<()> {
		...
		assert_eq!(
			format!("{reader:?}"),
			"VersaTilesReader { parameters: TileSourceMetadata { tile_compression: Gzip, tile_format: MVT, traversal: Traversal(AnyOrder,1..256), preferred_traversal: None, tile_pyramid: RwLock { data: None, poisoned: false, .. } } }"
		);
		assert_wildcard!(
		...
		assert_eq!(
			...
			"TileSourceMetadata { tile_compression: Gzip, tile_format: MVT, traversal: Traversal(AnyOrder,1..256), preferred_traversal: None, tile_pyramid: RwLock { data: None, poisoned: false, .. } }"
		);
