...
mod tests {
	...
	async fn read_your_own_dog_food() -> Result<()> {
		...
		VersaTilesWriter::write_to_writer(&mut reader1, &mut data_writer1, WriterConfig::default()).await?;
		...
		VersaTilesWriter::write_to_writer(&mut reader2, &mut data_writer2, WriterConfig::default()).await?;
