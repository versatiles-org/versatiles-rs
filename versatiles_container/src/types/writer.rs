...
use crate::TilesReaderTrait;
use anyhow::Result;
use async_trait::async_trait;
use std::{path::Path, sync::Arc};
use versatiles_core::{TileCompression, config::Config, io::*};
...
pub trait TilesWriterTrait: Send {
	...
	async fn write_to_path(
		...
		Self::write_to_writer(reader, &mut DataWriterFile::from_path(path)?, compression, config).await
	}
