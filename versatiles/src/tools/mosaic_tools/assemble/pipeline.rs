...
mod tests {
	...
	fn prepare_batches_packs_disjoint_tiles_densely() {
		...
		let batches = prepare_batches(map, HashSet::new(), 256, max_buffer);
	...
	fn prepare_batches_returns_none_when_empty() {
		...
		assert!(prepare_batches(map, done, 256, 1_000_000).is_empty());
	}
	...
	fn prepare_batches_prunes_done_coords() {
		...
		let batches = prepare_batches(map, done, 256, 0);
	...
	fn prepare_batches_respects_buffer_size() {
		...
		let batches = prepare_batches(map, HashSet::new(), 256, 300_000);
	...
	fn prepare_batches_large_tile_dim() {
		...
		let batches = prepare_batches(map, HashSet::new(), 512, 2_000_000);
	...
	fn prepare_batches_small_tile_dim() {
		...
		let batches = prepare_batches(map, HashSet::new(), 64, 50_000);
		let total: usize = batches.iter().map(TileBatch::len).sum();
		assert_eq!(total, 7);
		for batch in &batches {
	...
	fn prepare_batches_max_buffer_one_byte() {
		...
		let map = make_translucent_map(&[(tc(0, 0, 0), &[0]), (tc(1, 0, 0), &[1])]);
		let batches = prepare_batches(map, HashSet::new(), 256, 1);
