...
impl Blob {
	pub fn from_vec(vec: Vec<u8>) -> Blob { Blob(Bytes::from(vec)) }
	pub fn from_slice(slice: &[u8]) -> Blob { Blob(Bytes::copy_from_slice(slice)) }
	pub fn empty() -> Blob { Blob(Bytes::from(Vec::new())) }
