...
- *`level_base`: u8 (optional)* - The zoom level to use as the source for overzooming, tiles at this level and below are passed through unchanged, but tiles above this level are generated by clipping and rescaling features. Defaults to the maximum zoom level of the source.
- *`level_max`: u8 (optional)* - The maximum zoom level to support. Requests above this level will not return tiles. Defaults to `30`.
- *`enable_climbing`: bool (optional)* - If `true`, the operation will search parent tiles at lower zoom levels until it finds an existing tile, then clip and rescale from there. Defaults to `false`.
